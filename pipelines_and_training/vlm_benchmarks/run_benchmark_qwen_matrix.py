...
def main():
    ...
    parser.add_argument('--models', type=str, default='lora_v2', choices=['lora_v2', 'lora_v2_enhanced', 'base_v2', 'lora_v1', 'all'], help='Model to benchmark')
    parser.add_argument('--max_samples', type=int, default=None, help='Limit number of samples for testing')
    args = parser.parse_args()

    # 1. Đọc và chuẩn hóa CSV hiện có
    if os.path.exists(OUTPUT_CSV):
        df_existing = pd.read_csv(OUTPUT_CSV)
        # Chuẩn hóa tên Qwen3-VL gốc nếu cần
        if 'Qwen3-VL (8B)' in df_existing['Model'].values:
            df_existing['Model'] = df_existing['Model'].replace({'Qwen3-VL (8B)': 'Qwen3-VL (8B) - Base (Prompt v1)'})
            df_existing.to_csv(OUTPUT_CSV, index=False)
            print("Đã chuẩn hóa tên mô hình cũ: 'Qwen3-VL (8B)' -> 'Qwen3-VL (8B) - Base (Prompt v1)'")
        existing_keys = set(zip(df_existing['Sample_Idx'], df_existing['Model']))
    else:
    ...
    print("=" * 90)
    print(f"🚀 KHỞI ĐỘNG BENCHMARK MA TRẬN CHO HỌ QWEN3-VL ({total_samples} MẪU)")
    print(f"   Target Model: {args.models.upper()}")
    ...
    for model_display_name, model_type in models_to_run:
        print(f"\n=========================================================================")
        print(f"🏁 ĐANG ĐÁNH GIÁ: {model_display_name}")
        print(f"=========================================================================")

        # Chuẩn bị model nếu là lora_v1 trong local process
        hf_model = None
        hf_processor = None
        if model_type == 'lora_v1':
            print("Đang nạp mô hình LoRA v1 vào bộ nhớ...")
        ...
        for i, line in enumerate(all_lines):
            ...
            # Bỏ qua nếu mẫu đã được đánh giá
            if (i, model_display_name) in existing_keys:
                continue
            ...
            gt_raw = sample['messages'][1]['content']
            gt_json = parse_json(gt_raw) or {}
            ...
            if model_type == 'lora_v2':
                ...
                try:
                    ...
                    t0 = time.time()
                    raw_out = call_ollama('qwen3-vl:8b-instruct', PROMPT_SCHEMA_V2, img_b64=b64)
                    latency = round(time.time() - t0, 2)
                    pred_json = parse_json(raw_out)
                except Exception as e:
                    print(f"Lỗi đọc ảnh {img_path}: {e}")
            
            elif model_type == 'lora_v1':
                # Suy luận với hf_model LoRA v1
                import torch
                pil_img = Image.open(img_path).convert("RGB")
                w, h = pil_img.size
                if max(w, h) > 1280:
        ...
        print(f"\n✅ Hoàn thành đánh giá: {model_display_name}!")
