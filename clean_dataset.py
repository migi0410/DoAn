...
FIELD_ALIASES = {'STORE_NAME': 'SELLER', 'SHOP_NAME': 'SELLER', 'MERCHANT': 'SELLER', 'DATE': 'TIMESTAMP', 'TIME': 'TIMESTAMP', 'DATETIME': 'TIMESTAMP', 'TOTAL_AMOUNT': 'TOTAL_COST', 'TOTAL': 'TOTAL_COST', 'AMOUNT': 'TOTAL_COST', 'GRAND_TOTAL': 'TOTAL_COST', 'ITEM_QUANTITY': 'ITEM_QTY', 'QTY': 'ITEM_QTY', 'SL': 'ITEM_QTY'}

def normalize_json_response(raw_text):
    json_str = None
    if '```json' in raw_text:
        m = re.search('```json\\s*(.*?)\\s*```', raw_text, re.DOTALL)
        if m:
            json_str = m.group(1).strip()
    elif '```' in raw_text:
        m = re.search('```\\s*(.*?)\\s*```', raw_text, re.DOTALL)
        if m:
            json_str = m.group(1).strip()
    else:
        start = raw_text.find('{')
    ...
    with open(input_file, 'r', encoding='utf-8') as fin, open(output_file, 'w', encoding='utf-8') as fout:
        for line in fin:
            ...
            try:
                ...
                if '```json' in raw_json:
                    m = re.search('```json\\s*(.*?)\\s*```', raw_json, re.DOTALL)
                    if m:
                        raw_json = m.group(1)
            ...
            kept += 1
    print(f'  [{split_name:5s}] {total:>5} in -> {kept:>5} kept | skip_parse={skipped_no_json} skip_empty={skipped_no_fields} renamed={field_renames}')
    return kept
