...
from transformers import RobertaTokenizerFast, AutoModelForTokenClassification, TrainingArguments, Trainer, DataCollatorForTokenClassification
import evaluate
...
def load_data_1d(metadata_path, base_dir):
    ...
    return {'id': list(range(len(all_words))), 'words': all_words, 'ner_tags': all_ner_tags}
...
def main():
    ...
    features = Features({'id': Value(dtype='int64'), 'words': Sequence(Value(dtype='string')), 'ner_tags': Sequence(ClassLabel(names=LABELS))})
    ...
    def tokenize_and_align_labels(examples):
        tokenized_inputs = tokenizer(examples['words'], padding='max_length', truncation=True, is_split_into_words=True, max_length=256)
        labels = []
    ...
    model = AutoModelForTokenClassification.from_pretrained(args.model_name, num_labels=len(LABELS), id2label=id2label, label2id=label2id)
    ...
    training_args = TrainingArguments(output_dir='./phobert-avir-kie', max_steps=50 if args.smoke_test else -1, num_train_epochs=epochs, per_device_train_batch_size=16, per_device_eval_batch_size=16, eval_strategy='epoch', save_strategy='epoch', learning_rate=3e-05, logging_steps=50, load_best_model_at_end=True, metric_for_best_model='f1')
    data_collator = DataCollatorForTokenClassification(tokenizer)
    trainer = Trainer(model=model, args=training_args, train_dataset=train_tokenized, eval_dataset=val_tokenized, processing_class=tokenizer, data_collator=data_collator, compute_metrics=compute_metrics)
