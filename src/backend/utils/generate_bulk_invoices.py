...
def run_bulk_generator(count=2500, output_dir='synthetic_dataset', clean_ratio=0.2, mcocr_mode=True, backgrounds=True, save_pdfs=False, start_idx=1):
    ...
    templates_keys = ['einvoice_viettel', 'einvoice_vnpt', 'receipt_c45_bb', 'supermarket_winmart', 'supermarket_lotte', 'supermarket_bachhoaxanh', 'convenience_circlek', 'convenience_gs25', 'convenience_7eleven', 'cafe_highlands', 'cafe_phuclong', 'cafe_starbucks', 'restaurant_kfc', 'restaurant_jollibee', 'minimart_anan']
    ...
    dirs = {}
    for split in splits:
        dirs[split] = {'images': os.path.join(output_dir, split, 'images'), 'labels': os.path.join(output_dir, split, 'labels')}
        if save_pdfs:
        ...
        for t_idx, template_key in enumerate(templates_keys):
            ...
            for split, run_idx in runs:
                ...
                if perspective > 0 or shadow > 0 or flash > 0 or (fold > 0) or (fade > 0) or (streak > 0) or (bg_type != 'white'):
                    try:
                        img_augmented, box_data_augmented = ReceiptAugmenter.apply_pipeline(temp_png_path, box_data, perspective_level=perspective, shadow_level=shadow, flash_level=flash, fold_level=fold, fade_level=fade, streak_level=streak, bg_type=bg_type)
                ...
                if mcocr_mode:
                    mapped_box_data = []
                    label_map = {'store_name': 'SELLER', 'address': 'ADDRESS', 'date': 'TIMESTAMP', 'total_amount': 'TOTAL_COST'}
                    for ann in box_data:
                ...
                label_data = {'file_name': f'{prefix}.png', 'image_width': viewport_width, 'image_height': image_height, 'annotations': box_data}
                with open(json_path, 'w', encoding='utf-8') as f:
                    json.dump(label_data, f, ensure_ascii=False, indent=2)
                meta_item = {'id': total_generated, 'file_name': f'{prefix}.png', 'template': template_key, 'store_name': invoice_data['store_name'], 'total_amount': invoice_data['total_amount'], 'image_path': f'{split}/images/{prefix}.png', 'label_path': f'{split}/labels/{prefix}.json', 'pdf_path': f'{split}/pdfs/{prefix}.pdf' if save_pdfs else None, 'is_a4': is_a4, 'is_c45': is_c45, 'augmentations': {'perspective': round(perspective, 3), 'shadow': round(shadow, 3), 'flash': round(flash, 3), 'fold': round(fold, 3), 'fade': round(fade, 3), 'streak': round(streak, 3), 'background': bg_type}}
                if split == 'train':
...
if __name__ == '__main__':
    ...
    run_bulk_generator(count=args.count, output_dir=args.output_dir, clean_ratio=args.clean_ratio, mcocr_mode=mcocr_mode, backgrounds=backgrounds, save_pdfs=args.save_pdfs, start_idx=args.start_idx)
