...
FOLDERS_TO_PROCESS = ['cafe_starbucks', 'supermarket_bachhoaxanh', 'cafe_highlands', 'cafe_phuclong', 'convenience_7eleven', 'supermarket_winmart', 'einvoice_viettel', 'receipt_c45_bb']
MAX_IMAGES = 99999
...
def run_paddle_ocr(image_bytes):
    ...
    if result and result[0]:
        for idx, line in enumerate(result[0]):
            ...
            ocr_texts.append({'paddle_id': idx, 'text': txt, 'box_2d': norm_box})
...
def call_ollama_hybrid(img, ocr_texts):
    ...
    payload = {'model': 'minicpm-v', 'prompt': PROMPT, 'images': [img_b64], 'stream': False, 'format': 'json', 'options': {'temperature': 0.1}}
                    ...
                    if data and 'paddle_id' in data:
                        ...
                        for t in ocr_texts:
                            if t['paddle_id'] == pid:
                                box = t['box_2d']
                                break
                        final_data[field] = {'text': data.get('text'), 'box_2d': box}
                    else:
