  ...
  try {
    ...
    if (sampleId && PRESET_CORNERS[sampleId]) {
      ...
      return NextResponse.json({
        ...
        steps: [
          `1. Dò 4 góc viền bằng FastViT Heatmap: P1(${corners[0].x}%, ${corners[0].y}%), P2(${corners[1].x}%, ${corners[1].y}%), P3(${corners[2].x}%, ${corners[2].y}%), P4(${corners[3].x}%, ${corners[3].y}%)`,
    ...
    if (file) {
      ...
      return NextResponse.json({
        ...
        is_live_inference: true,
        steps: [
          `1. DocAligner phát hiện 4 góc tài liệu: P1(${x1}%, ${y1}%), P2(${x2}%, ${y2}%), P3(${x3}%, ${y3}%), P4(${x4}%, ${y4}%)`,
    ...
    return NextResponse.json({ success: false, error: error.message || "Lỗi xử lý DocAligner" }, { status: 500 });
  }
}
